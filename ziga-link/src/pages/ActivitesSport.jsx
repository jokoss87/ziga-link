...
import { SPORT_TYPES } from "@/components/lib/activityTypeConstants";

// SPORT_TYPES importé depuis activityTypeConstants

export default function ActivitesSport() {
  ...
  useEffect(() => {
    const unsub = base44.entities.Activity.subscribe((event) => {
      if (event.type === "create" && event.data && SPORT_TYPES.includes(event.data.type) && event.data.status === "open") {
  ...
  const loadData = async () => {
    ...
    setActivities(all.filter(a => SPORT_TYPES.includes(a.type)));
  ...
  const sportConfigs = activeConfigs.filter(cfg => SPORT_TYPES.includes(cfg.type_key));
  const filtered = filter === "all" ? activities : activities.filter((a) => a.type === filter);

  return (
    <div className="min-h-screen bg-stone-50 pb-20">
      <div className="px-6 py-10 text-white" style={{ background: CAT.sport.gradient }}>
        <div className="max-w-2xl mx-auto">
          <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-orange-100 hover:text-white mb-4 text-sm">
            <ArrowLeft className="w-4 h-4" /> Retour
          </button>
          <h1 className="text-3xl font-black mb-1">🏃‍♂️ Activités sportives</h1>
          <p className="text-orange-100 mb-5">Sports canins avec la communauté</p>
          <Button
            onClick={() => setShowCreate(true)}
            className="bg-white text-orange-600 hover:bg-orange-50 font-bold gap-2 rounded-xl shadow-lg"
          >
      ...
      <div className="max-w-2xl mx-auto px-4 py-5">
        {user && myRequests.length > 0 && (
          <div className="mb-6 bg-orange-50 rounded-2xl p-4 border border-orange-200">
                ...
                return (
                  <div key={req.id} className="bg-white rounded-lg p-3 text-sm">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <span className="font-medium text-orange-900">{act?.title || "Activité"}</span>
                    ...
                    {expandedRequests[req.id] && (
                      <div className="mt-2 pt-2 border-t border-orange-100 space-y-1 text-xs text-stone-600">
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 mb-4">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap border transition-all ${filter === "all" ? "bg-orange-500 text-white border-orange-500 shadow-md" : "bg-white text-stone-600 border-stone-200 hover:border-orange-300"}`}
          >
            🌟 Toutes
          </button>
          {sportConfigs.map(cfg => (
            <button
              key={cfg.type_key}
              onClick={() => setFilter(cfg.type_key)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium whitespace-nowrap border transition-all ${filter === cfg.type_key ? "bg-orange-500 text-white border-orange-500 shadow-md" : "bg-white text-stone-600 border-stone-200 hover:border-orange-300"}`}
            >
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-stone-500 bg-white rounded-2xl border border-stone-100">
            ...
            <p className="font-semibold text-stone-700">Aucune activité sportive ouverte</p>
            <p className="text-sm text-stone-400 mt-1 mb-5">Créez la première !</p>
            <Button onClick={() => setShowCreate(true)} className="bg-orange-500 hover:bg-orange-600 text-white rounded-xl gap-2">
              <Plus className="w-4 h-4" /> Créer une activité
            </Button>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.slice(0, visibleCount).map((act) => (
              <div key={act.id} id={`activity-${act.id}`} className={highlightId === act.id ? "ring-2 ring-orange-400 rounded-2xl" : ""}>
            ...
            {visibleCount < filtered.length && (
              ...
              >
                Voir plus ({filtered.length - visibleCount} activités)
              </button>
            )}
          </div>
        )}
